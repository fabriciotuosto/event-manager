...
public class UserTest {
	...
	public void create_user_with_group() {
		...
		User user = User.newUser("nombre", "valid@email.co", "1234aA").inGroup(
				group).build();
		assertNotNull(user);
	...
	@Test
	@SuppressWarnings("deprecation")
	public void add_contacts_retrieve_same() {
		User user = new User(ID);
		User contact = new User(ID);
		contact.setName("Developer");
		user.addContact(contact, contact);
	...
	public void should_record_unresponded_user(){
		Event event = new Event(ID);
		User user = new User(ID);
		event.invite(user);
